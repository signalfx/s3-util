...
module.exports = function putStream(client, stream, destinationPath, options){
  ...
  stream.on('end', function(){
    // If this is zero byte, don't attempt to upload since it will fail
    if(contentLength === 0) {
      deferred.resolve();
      return;
    }
    ...
    client.putStream(streamBuffer, destinationPath, headers, function(err, res){
      if(err) {
        return deferred.reject({
          message: 'Unable to upload to destination: ' + destinationPath,
          error: err
        });
      }
      ...
      res.on('end', function(){
        if(res.statusCode >= 300){
          ...
          deferred.reject({
            ...
            error: new Error(destinationPath + ' returned status code of ' + res.statusCode + '.\n' + responseText + '\n')
          });
          res.resume();
        } else {
          deferred.resolve();
